...
+
您會發現每個RDM項目的整個RDM對應項目都是「_對應原始LUN_」。
...
.步驟
. 移至虛擬機器目錄。
. 切換到目錄「CD /vmfs/volumes/Data」儲存路徑。
+
[listing]
----
# ls -l
...
. 刪除所有內嵌SMVI字串的「vmdk」檔案。或者、您也可以使用LUN名稱來識別vmdk檔案。
+
或者、您也可以使用LUN名稱來識別「vmdk」檔案。

+
如果LUN名稱的「vmdk」檔案為「_rdm1_」、請僅刪除「_rhel4u8-141-232_SMVI_vol_esx3u5_rdm1-rdm.vmdk_」和「_rhel4u8-141-232_SMVI_vol_3u5_rdm1_esx1.vmdk檔案。

+
刪除「_vmlnx5U4-197-23_SMVI_10.72.197.93_C4koV4XzK2HT_22-rdmmp.vmdk_'及「_vmlnx5U4-197-23_SMVI_10.72.197.93_C4koV4XzK2HT_22.kvm5nv_4kvm_4kov_4kvm_4kvm_4kov_4kov_4kv_4km_41934_4kov_4kov_4kv_4kv_4kv_4kv41934_4kv_4

. 從虛擬機器組態檔（「vmx」）中移除「vmdk」檔案項目。
+
以下是從vmx檔案移除vmdk檔案項目的範例。
